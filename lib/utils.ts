...
export function generateSampleData(charts, rows, minValue, maxValue) {
  const data = {};

  // Initialize charts
  for (let chart = 1; chart <= charts; chart++) {
    data[`Chart ${chart}`] = {};
  }
  ...
  for (let chart = 1; chart <= charts; chart++) {
    for (let row = 0; row < rows; row++) {
      const value =
        Math.floor((Math.random() * (maxValue - minValue)) / 10) * 10 +
        minValue;

      // Introduce null values (20% probability)
      data[`Chart ${chart}`][row] = Math.random() < 0.8 ? value : null;
    }
  }
